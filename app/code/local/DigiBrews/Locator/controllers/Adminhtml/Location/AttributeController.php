...
class DigiBrews_Locator_Adminhtml_Location_AttributeController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    protected function _getEntityType()
    {
        if (is_null($this->_entityType)) {
            $this->_entityType = Mage::getSingleton('eav/config')->getEntityType(DigiBrews_Locator_Model_Location::ENTITY);
        }
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('digibrews_locator/attributes')
            ->_addBreadcrumb(
                Mage::helper('digibrews_locator')->__('Locator'),
                Mage::helper('digibrews_locator')->__('Locator'))
            ->_addBreadcrumb(
                Mage::helper('digibrews_locator')->__('Manage Location Attributes'),
                Mage::helper('digibrews_locator')->__('Manage Location Attributes'));
        return $this;
    }
    ...
    {
        $attribute = Mage::getModel('digibrews_locator/attribute');
    ...
    public function editAction()
    {
        ...
        if ($attributeId) {
            ...
            if (!$attributeObject->getId()) {
                $this->_getSession()
                    ->addError(Mage::helper('digibrews_locator')->__('Attribute is no longer exists.'));
                $this->_redirect('*/*/');
                return;
            }
            if ($attributeObject->getEntityTypeId() != $this->_getEntityType()->getId()) {
                $this->_getSession()->addError(
                    Mage::helper('digibrews_locator')->__('You cannot edit this attribute.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        $label = $attributeObject->getId()
            ? Mage::helper('digibrews_locator')->__('Edit Location Attribute')
            : Mage::helper('digibrews_locator')->__('New Location Attribute');
    ...
    public function validateAction()
    {
        ...
        if (!$attributeId) {
            ...
            if ($attributeObject->getId()) {
                $this->_getSession()->addError(
                    Mage::helper('digibrews_locator')->__('Attribute with the same code already exists')
                );
    ...
    {
        return Mage::helper('digibrews_locator/location')->filterPostData($data);
    }
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost() && $data) {
            ...
            $attributeObject = $this->_initAttribute();
            /* @var $helper DigiBrews_Location_Helper_Data */
            $helper = Mage::helper('digibrews_locator');
            ...
            if (isset($data['used_in_forms']) && is_array($data['used_in_forms'])) {
                $data['used_in_forms'][] = 'digibrews_locator_location';
            }
            ...
            try {
                Mage::dispatchEvent('digibrews_locator_attribute_before_save', array(
                    'attribute' => $attributeObject
                ));
                $attributeObject->save();
                Mage::dispatchEvent('digibrews_locator_attribute_save', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                     Mage::helper('digibrews_locator')->__('The location attribute has been saved.')
                );
                ...
                $this->_getSession()->addException($e,
                     Mage::helper('digibrews_locator')->__('An error occurred while saving the location attribute.')
                );
                $this->_getSession()->setAttributeData($data);
                $this->_redirect('*/*/edit', array('_current' => true));
                return;
    ...
    public function deleteAction()
    {
        ...
        if ($attributeId) {
            ...
            {
                $this->_getSession()->addError(
                     Mage::helper('digibrews_locator')->__('You cannot delete this attribute.')
                );
                $this->_redirect('*/*/');
                return;
            }
            try {
                ...
                Mage::dispatchEvent('digibrews_locator_attribute_delete', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                     Mage::helper('digibrews_locator')->__('The location attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Mage_Core_Exception $e) {
                ...
                $this->_getSession()->addException($e,
                     Mage::helper('digibrews_locator')->__('An error occurred while deleting the location attribute.')
                );
                $this->_redirect('*/*/edit', array('attribute_id' => $attributeId, '_current' => true));
                return;
            }
