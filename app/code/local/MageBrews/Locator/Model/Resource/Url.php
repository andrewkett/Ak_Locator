...
class MageBrews_Locator_Model_Resource_Url extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    {
        return Mage::getSingleton('magebrews_locator/location');
    }
    ...
    {
        //return array();
        //die('todo - change this function to locations');
        ...
        if (!isset($this->_locationAttributes[$attributeCode])) {
            ...
            //echo 'attribute code '.$attributeCode;
            // print_r($attribute);
        ...
        unset($rowSet);
        foreach ($locationIds as $locationIds) {
            if (!isset($attributes[$locationIds])) {
                $attributes[$locationIds] = null;
            }
        }
    ...
    protected function _getLocations($locationIds, $storeId, $entityId, &$lastEntityId)
    {
        ...
        $select = $adapter->select()
            ...
            ->from(array('e' => $this->getTable('magebrews_locator/location')), array('entity_id'))
            // ->join(
            //     array('w' => $this->getTable('magebrews_locator/location_website')),
     ...
     * @return array
     */
    public function getLocationsByStore($locationId, &$lastEntityId)
    {
        return $this->_getLocations(null, $locationId, $lastEntityId, $lastEntityId);
    }
     ...
     *
     * @param int $locationId Location entity Id
     * @param int $storeId Store Id for rewrites
     * @param array $excludeCategoryIds Array of category Ids that should be skipped
     * @return Mage_Catalog_Model_Resource_Url
     */
    public function clearProductRewrites($locationId, $storeId, $excludeCategoryIds = array())
    {
        $where = array(
